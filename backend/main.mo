...
actor {
  ...
  public type BulkEmployeeRecord = {
    employeeId : Text;
    internalId : Nat;
    status : Text;
    businessEmail : Text;
  ...
  public shared ({ caller }) func addEmployee(details : EmployeeDetails) : async Text {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Runtime.trap("Unauthorized: Only users can add employees");
    };
    if (not isHROrAdmin(caller)) {
      Runtime.trap("Unauthorized: Only HR staff and admins can add employees");
    };
  ...
  public shared ({ caller }) func bulkCreateEmployees(bulkRecords : [BulkEmployeeRecord]) : async [BulkCreateStatus] {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
    ...
    let results = Array.tabulate(
      bulkRecords.size(),
      func(i) {
        let record = bulkRecords[i];

        switch (employees.containsKey(record.employeeId)) {
          case (true) {
            {
          ...
          case (false) {
            ...
            let leaveBalance : LeaveBalance = {
              annual = record.annualLeave;
              sick = record.sickLeave;
              casual = record.casualLeave;
              unpaid = record.unpaidLeave;
            };

            employees.add(record.employeeId, profile);
            leaveBalances.add(record.employeeId, leaveBalance);
